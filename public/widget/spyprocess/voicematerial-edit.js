...
define([
    ...
    function init(initParam){
        ...
        $.getJSON('/spycommon/getFieldIdAndFieldNameMap', {
            ...
            if(rsp.code != 0)
                return notify.show({
                    title: i18n.t('casemanage.spyProcess.materialeditinfo.gettabletransdicerr'),
                    type: 'error'
                });
    ...
    function render(opts){
        ...
        $('.summernote').summernote({
            ...
            placeholder: i18n.t('casemanage.spyProcess.materialeditinfo.editcontent'),
            lang: 'zh-CN',
            height: _opts.height,
    ...
    function saveVoiceMaterialData(optUrl, paramData, callback){
        $.ajax({
            ...
            'success': function(result){
                ...
                if(result.code != 0)
                    return notify.show({
                        title: i18n.t('casemanage.spyProcess.materialeditinfo.savefileerror'),
                        type: 'error'
                    });
        ...
        digestInit(getDigestDetail(_materialContentTable), 100007, function(code, digestDesc){
            ...
            if(code == 0)
                ...
                function(res, status){
                    ...
                    if(res.code != 0)
                        return notify.show({
                            title: i18n.t('casemanage.spyProcess.materialeditinfo.savedataerror'),
                            type: 'error',
                        });
    ...
    function getObjectDetailInfo(objectId){
        if(objectId == undefined)
            return notify.show({
                title: i18n.t('casemanage.spyProcess.materialeditinfo.getobjctinfoerror'),
                type: 'error'
            });
        ...
        $.ajax({
            ...
            'success': function(result){
                ...
                if(result.code != 0)
                    return notify.show({
                        title: i18n.t('casemanage.spyProcess.materialeditinfo.getobjectinfoerror'),
                        type: 'error',
                    });
    ...
    function getCaseDetailInfo(caseId){
        if(caseId == undefined)
            return notify.show({
                title: i18n.t('casemanage.spyProcess.materialeditinfo.getcaseinfoerror'),
                type: 'error',
            });
    ...
    function getMaterialInfo(cdrId, callback){
        if(cdrId == undefined)
            return notify.show({
                title: i18n.t('casemanage.spyProcess.materialeditinfo.getcontenterror'),
                type: 'error',
            });
        ...
        $.ajax({
            ...
            'success': function(result){
                ...
                if(result.data.forms.length == 0){
                    ...
                    return notify.show({
                        title: i18n.t('casemanage.spyProcess.materialeditinfo.contentdeleted'),
                        type: 'alert'
                    });
                }
    ...
    function getMaterialContent(recordId){
        if(recordId == undefined)
            return notify.show({
                title: i18n.t('casemanage.spyProcess.materialeditinfo.getcontentidnull'),
                type: 'error'
            });
    ...
    function genVoiceMaterialParam(){
        ...
        if(_materialContentTable.length == 0){
            ...
            _materialContentTable = [{
                ...
                fieldValue: _caseInfo == undefined ? '' : (_caseInfo.type == 0 ? i18n.t('casemanage.spyProcess.materiallabel.ordinarytype'):i18n.t('casemanage.spyProcess.materiallabel.specialtype'))
            },{
    ...
    function digestInit(data, tableType, callback){
        ...
        $.get('/tabledesign/getInitTableData?tableId=' + tableType).done(function(res){
            ...
            if(result.code == 0){
                for(var key in data){
                    if(key == 'CASE_NAME')
                        digestDesc += ',' + i18n.t('casemanage.spyProcess.materiallabel.casename') + data[key];
