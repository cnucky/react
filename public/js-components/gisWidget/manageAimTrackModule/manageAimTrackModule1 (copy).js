...
(function(window, document, undefined) {
        ...
        $('[name=selectedAim]:checkbox').click(function() {
            ...
            } else //若取消对目标的选中
            {
                ...
                if ($("." + tabHeader).hasClass("activeTab")) {
                    ...
                    _tabClickChange(currentActiveTab, preActiveTab,currentActiveTab.attr("aimId")); //调用tab转换方法
    ...
    function _addAimTabClickEvent() {
        $(".tabTitle").click(function() {
            ...
            if (currentActiveTab == preActiveTab) return; //若点击的tab本就为活动状态，则直接返回
    ...
    function _setPlayerStatus(isReplay, isEnding, isInit, beginTime, endTime, leftPercent, rightPercent, totalNum) {
        if (isReplay) //若是“重放”
        {
            $("#track-player-status")[0].innerHTML = "轨迹播放"; //修改状态提示
            $("#trackPlayerPlay").hide(); //隐藏“播放”键
            $("#trackPlayerPause").show(); //显示“暂停”键
            $("#track-player-progressTip")[0].innerHTML = "当前播放速度1x"; //设置重放速度
        ...
        if (isEnding) //若是播放结束
        {
            $("#track-player-status")[0].innerHTML = "播放完毕"; //修改状态提示
