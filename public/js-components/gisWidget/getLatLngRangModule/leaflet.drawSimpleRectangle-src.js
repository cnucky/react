...
(function (window, document, undefined) {
    //绘制线段
    function MyLatLngRectangleRange(menu){  //MyDrawRectangle(options,editMenu)
        this.options={
            name:"DrawRectangle",
            repeatMode: false,
    ...
    MyLatLngRectangleRange.prototype={
        type:'rectangle',
        addTo: function (map) {
              this._map = map;
        },
        ...
        _fireCreatedEvent: function (shapeOptions) {
            var bound = this._shape.getBounds();
            var rectangle = new L.Rectangle(bound, shapeOptions);
            rectangle.graphID=this._shape.graphID;
            this.options.xmax = bound.getEast().toFixed(4);
            this.options.xmin = bound.getWest().toFixed(4);
            this.options.ymax = bound.getNorth().toFixed(4);
            this.options.ymin = bound.getSouth().toFixed(4);
            ...
            L.DomEvent.on(rectangle,'contextmenu',function(e){
                ...
                if(this.editPopup._parent!=undefined){
                    //不同图形之间切换处理
                    if(!(this.editPopup._parent instanceof MyLatLngRectangleRange)) {
...
/**********************************************************************************************************************/
        ...
        _saveMenu:function(){
            this.editPopup.style.display="none";
            var bound = this.currentTarget.getBounds();
            this.options.xmax = bound.getEast().toFixed(4);
            this.options.xmin = bound.getWest().toFixed(4);
            this.options.ymax = bound.getNorth().toFixed(4);
            this.options.ymin = bound.getSouth().toFixed(4);
            this._map.fire('draw:changed', { layerType: this.type });
            this._finishEditSingleRect();
        },
        _deleteMenu:function(){
            this.currentTarget._map.removeLayer(this.currentTarget);
            this.options.xmax = null;
            this.options.xmin = null;
            this.options.ymax = null;
            this.options.ymin = null;
...
//                .off('MSPointerUp', this._onTouchEnd, this);
    ...
    myLatLngRectangleRange=function(menu){
        return new MyLatLngRectangleRange(menu);
    }
}(window, document));