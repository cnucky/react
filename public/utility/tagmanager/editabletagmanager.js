...
(function($) {
    ...
    publicMethods = {
        pushTag : function (tag, ignoreEvents, externalTagId, tagAttr, editFunction, removeFunction) {
            var $self = $(this), opts = $self.data('opts'), alreadyInList, tlisLowerCase, max, tagId,
            tlis = $self.data("tlis"), tlid = $self.data("tlid"), idx, newTagId, newTagRemoveId, newTagEditId, escaped,
            ...
            if (alreadyInList) {
                ...
                newTagEditId = $self.data("tm_rndid") + '_Editor_' + tagId;

                ////for demo 
                if (tagAttr['tagOperate'] == '=') {
                    var displayContent = tag + '   (' + tagAttr['tagValue'] + ')';
                    escaped = $("<span/>").text(displayContent).html();

                } else {
                    var displayContent = tag + '   (' + tagAttr['tagHead'] + tagAttr['tagOperate'] + tagAttr['tagValue'] + ')';
                    escaped = $("<span/>").text(displayContent).html();
                }
                
                html = '<span class="' + privateMethods.tagClasses.call($self) + '" id="' + newTagId + '" tagKey=' + tagAttr['tagKey'] + '>';
                html+= '<span style="cursor:pointer;"><a style="color:#fff;" id="' + newTagEditId + '" TagIdToEdit="' + tagId + '">' + escaped + '</a></span>';
                ...
                $el.find("#" + newTagRemoveId).on("click", $self, function(e) {
                    ...
                    removeFunction($self, TagIdToRemove);
                    privateMethods.spliceTag.call($self, TagIdToRemove, e.data);
                });

                $el.find("#" + newTagEditId).on("click", $self, function(e) {
                    e.preventDefault();
                    var TagIdToEdit = parseInt($(this).attr("TagIdToEdit"));
                    editFunction($self, TagIdToEdit);
                    //privateMethods.switchTag.call($self, TagIdToRemove, e.data);
                });
    ...
    privateMethods = {
        ...
        spliceTag : function (tagId) {
            var $self = this, opts = $self.data('opts'), tlis = $self.data("tlis"), tlid = $self.data("tlid"), idx = $.inArray(tagId, tlid),
            tagBeingRemoved;
            ...
            if (-1 !== idx) {
                ...
                if (!opts.handleSpliceTag || !opts.handleSpliceTag(tagId, action)) {
                    action();
                }
        ...
        switchTag : function (tagId) {
            ...
            if (-1 !== idx) {
                var action = function() {
                    ...
                    $("#" + $self.data("tm_rndid") + "_" + tagId).attr("class", "tm-tag tm-tag-danger");
