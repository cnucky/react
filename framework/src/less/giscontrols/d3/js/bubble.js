var bubblecloud={
    ...
    Bubbles : function(width,height) {
      var chart, clear, click, collide, collisionPadding, color, connectEvents, data, force, gravity, hashchange, height, idValue, jitter, label, margin, maxRadius, minCollisionRadius, mouseout, mouseover, node, rScale, rValue, textValue, tick, transformData, update, updateActive, updateLabels, updateNodes, width;
      width = width;
      height = height;
      data = [];
      node = null;
      ...
      updateLabels = function() {
        ...
        label.style("font-size", function(d) {
          return Math.max(7.5, rScale(rValue(d) / 2.5)) + "px";
      ...
      click = function(d) {
        bubblecloud.dispatchEvent("click",idValue(d));
    ...
    display: function(id, data,width,height){
        return d3.select(id).datum(data).call(this.Bubbles(width,height));
    },
