define("module/gis/enclosureDrawCircle", [
    "jquery",
    "underscore",
], function() {
    var enclosureObj;
    ...
    MyDrawCircle.prototype = {
        ...
        _shapeName: function(source, graphicName) {
            ...
            var icon = new L.DivIcon({
                html: '<div><span>' + graphicName + '</span></div>',
                className: 'shapeName',
        ...
        //加载图形
        loadShape: function(graphicObj, graphicName, color, islocate) {
            if (!graphicObj) {
                return;
            } else {
        ...
        _setMenu: function(e, source) {
            this.editPopup.style.left = e.originalEvent.clientX + "px";
            this.editPopup.style.top = e.originalEvent.clientY + "px";
        ...
        _saveMenu: function() {
            ...
            //调用接口，保存图形
            enclosureObj.SaveEnclosure(isNew, '1', JSON.stringify({
                center: [this.currentTarget.getLatLng().lat, this.currentTarget.getLatLng().lng],
                radius: this.currentTarget.getRadius()
            }), this.currentTarget.graphID);
            if (isNew == false) {
                this._finishEditSingleCircle();
            }
        },
        _deleteMenu: function() {
            if (!this.currentTarget.graphID) {
        ...
        _finishEditSingleCircle: function() {
            document.getElementById("popEdit").children[0].children[1].innerHTML = "编辑";
            this.editPopup.style.display = "none";
            this._unbindMenu();
