...
var TableComponent = React.createClass({
	...
	render: function() {
		...
		columnNames = columnNames || [i18n.t("input-field"), i18n.t("type"), i18n.t("output-field")];

		var table = _.map(this.state.dataTable, _.bind(function (item,index) {
			return (
				<tr data-index={index} key={index + 2}>
					<td  className="p6">
						<input type="checkbox"
					...
					<td  className="p6">
						{this.props.hideOutputs ? '' : <input type="text" className="form-control pl5 pr5" style={{padding:"0px",height:"25px"}}
				...
				<table className="table table-hover">
					<thead>
						<tr className="primary" key={1}>
							...
							<th className="sortable">{this.props.hideOutputs ? '' : columnNames[2]}</th>
						</tr>
					</thead>
					<tbody>
						{table}
