...
class SystemAuthority extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            roleId:this.props.userRoleMember,
            roleName:this.props.userRoleName,
            systemCheckData:this.props.checkData,
            systemNotData:this.props.notData
        };
    }
    ...
    showRoleDetail(roleId,roleName) {
        const container = $('#system-tab');
        let checkData=this.state.systemCheckData;
        let notData=this.state.systemNotData;
        Tree.build({
            container:container ,
            selectMode: 3,
            minExpandLevel: 3,
            ...
            select: function(event, data) {
                ...
                if (data.node.isSelected() && data.node.data.selectedSubPermission[0] != i18n.t('rolemanage.level-none')) {
                    ...
                    store.dispatch({type:'SYSTEMCHECKDATA_GET',systemCheckData:checkData});
    ...
    render() {
        ...
        return (
            <div>
                <div id="system-tab" className="authority-tab"></div>
                <div className="bton">
