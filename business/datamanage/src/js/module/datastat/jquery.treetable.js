...
(function ($) {
    ...
    if (document.location.href.indexOf('http://') != 0)	{
        var path = '/datamanage/datastat/script/treeTable/';
    } else {
        var path = SITE_URL + '/datamanage/datastat/script/treeTable/';
    }
    ...
    $.fn.treeTable = function (opts) {
        ...
        if ($('head').find('#tree_table_' + opts.theme).length == 0) {
            $('head').append('<link id="tree_table_' + opts.theme + '"');
        }
        //$('head').append('<link id="tree_table_' + opts.theme + '"');
        ...
        function clearAllSelectedClass(treeTable, cssclass){
            try{
                //console.log("treeTable:", treeTable.rows.length);
                for(var i=0; i<treeTable.rows.length; ++i){
                    treeTable.rows[i].classList.remove(cssclass);
                }
            }catch(e){
                //console.log("clearAllSelectedClass error");
            }
        }
        ...
        $treeTable.unbind();
        $treeTable.bind("click", treeTableClick);
        function treeTableClick(event) {
            var $target = $(event.target);
            if($target[0].tagName == 'TD' || $target[0].tagName == 'SPAN') {
                clearAllSelectedClass($treeTable[0], 'clicked');
                clearAllSelectedClass($treeTable[0], 'hovered');
                if($target[0].tagName == 'TD')
                    $target.context.parentNode.classList.add('clicked');
                else
                    $target.context.parentNode.parentNode.classList.add('clicked');
            }
            ...
            if ($target.attr('arrow')) {
                ...
                if (className == css['AN'] + ' ' + css['HLO'] || className == css['AN'] + ' ' + css['HO'] || className == css['AN'] + ' ' + css['O']) {
                    var id = $target.parents('tr[haschild]')[0].id;
                    ...
                    shut(id);
                    return;
                } else if (className == css['AN'] + ' ' + css['HLS'] || className == css['AN'] + ' ' + css['HS'] || className == css['AN'] + ' ' + css['S']) {
        ...
        function hoverActiveNode(event) {
            var $target = $(event.target);
            //console.log($target[0].tagName+"  "+event.type);
            if(event.type == 'mouseout'){
                clearAllSelectedClass($treeTable[0], 'hovered');
                //console.log("clearAllSelectedClass");
            }

            if($target[0].tagName == 'TD' && event.type != 'mouseout'
                && !$target.context.parentNode.classList.contains('clicked')){
                clearAllSelectedClass($treeTable[0], 'hovered');
                $target.context.parentNode.classList.add('hovered');
                $target.context.parentNode.classList.add('hovered');
            }
