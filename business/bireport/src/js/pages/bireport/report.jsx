        ...
        return (
			<div id="top-toolbar" className="p5" style={{ position: 'relative' }}>
			    <Tooltip title="载入报表" placement="bottom">
					<button type="button" className="btn btn-default btn-sm" style={{ position: 'relative' }} onClick={this.changeTask}>
                ...
                <Tooltip title="下载" placement="bottom">
					<button type="button" className="btn btn-default btn-sm" onClick={this.handleDownload}>
...

				{
					!taskId && !modelId ? <div id="changeTask">
						<Tooltip title="选择任务" placement="bottom">
							<a className="large red button" style={{ position: 'absolute' ,right:'5px',top:'3px'}} onClick={this.changeTask}>请先选择任务</a>
						</Tooltip>
					</div> : ''
				}
...
function loadData(taskId,modelId) {
    let defer = Q.defer();
		...
		$.getJSON('/bireport/bireport/checkTask', {
			...
			if (rsp.code == 0) {
				if (rsp.data.reportId === -1){
					...
					manager.openReport(rsp.data.reportId).then(function(data){
						data.reportDetail.repotId = rsp.data.reportId;
...
if (taskId || modelId){
	loadData(taskId,modelId).then(function() {
