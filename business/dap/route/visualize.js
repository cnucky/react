...
//        };
    ...
    billDetailService(req).getBillData(args).then(function(rsp){
        mylogger.info('getpersonalturnoverdetail returned data :',JSON.stringify(rsp));
        var data = rsp.data;
        res.endj(data);
    })
        .catch(function(d) {
            mylogger.info('getpersonalturnoverdetail exception:',d);
            res.endj({});
        });
})
