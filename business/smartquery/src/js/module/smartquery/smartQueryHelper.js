define('queryHelper',['nova-notify', '../../widget/operator'], function(Notify, operator_map) {
    ...
    queryHelper.getCodeTable = function(fieldInfo) {
        $("#" + fieldInfo.fieldName).select2({
            ajax: {
                url: "/smartquery/smartquery/getCodeTable?typeId=" + fieldInfo.typeId + "&fieldName=" + fieldInfo.fieldName,
                dataType: 'json',
                delay: 250,
    ...
    queryHelper.inputCodeTable = function(fieldInfo) {
        $.getJSON('/smartquery/smartquery/getCodeTableByCode', fieldInfo).done(function(rsp) {
            if (rsp.data) {
    ...
    queryHelper.taskInput = function(taskId) {
        $.getJSON('/smartquery/smartquery/getTaskCondition', {
            'taskId': taskId
        }).done(function(rsp) {
